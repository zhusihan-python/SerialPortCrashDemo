...
public static class Crc16
{
    ...
    public static ushort CalcCrc(byte[] puchMsg, int startIndex, int usDataLen, bool highByteFirst = true)
    {
        byte uchCRCHi = 0xFF;
        byte uchCRCLo = 0xFF;
        uint uIndex;

        for (int i = startIndex; i < startIndex + usDataLen; i++)
        {
            uIndex = (uint)uchCRCHi ^ puchMsg[i];
            uchCRCHi = (byte)(uchCRCLo ^ auchCRCHi[uIndex]);
            uchCRCLo = auchCRCLo[uIndex];
        }
        return highByteFirst ? (ushort)(uchCRCHi << 8 | uchCRCLo) : (ushort)(uchCRCLo << 8 | uchCRCHi);
    }
    ...
    public static bool IsValidCrcInBracketPort(byte[] data)
    {
        ...
        int a = crc >> 8;
        int b = (data[^4] << 4) + data[^3];
        int c = (data[^2] << 4) + data[^1];
        int d = crc & 0xFF;
        return a == b && c == d;
    }
